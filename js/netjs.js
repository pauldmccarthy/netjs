...
define(
   ...
   * and edgeColourMax parameters, which specify the edge strength values,
   * normalised to lie between 0 and 1, that correspond to the minimum and
   * maximum edge colours - these are applied symmetrically to positive and
   * negative edge strengths.
   *
   *   - edgeColour
   *   - defEdgeColour
   *   - hltEdgeColour
   ...
   * can be overridden via the edgeWidthMin and edgeWidthMax parameters in a
   * similar manner to the edgeColourMin/edgeColourMax parameters.
   *
   *   - edgeWidth
   *   - defEdgeWidth
   *   - hltEdgeWidth
  ...
  function displayNetwork(network, display) {
    ...
    var netmin = network.matrixMins[network.thresholdIdx];
    var netmax = network.matrixMaxs[network.thresholdIdx];
    var netlen = netmax - netmin;
    ...
    if (display.edgeColourMin) vd.EDGE_COLOUR_MIN = netmin + display.edgeColourMin * netlen;
    if (display.edgeColourMax) vd.EDGE_COLOUR_MAX = netmin + display.edgeColourMax * netlen;
    ...
    if (display.edgeWidthMin) vd.EDGE_WIDTH_MIN = netmin + display.edgeWidthMin * netlen;
    if (display.edgeWidthMax) vd.EDGE_WIDTH_MAX = netmin + display.edgeWidthMax * netlen;
